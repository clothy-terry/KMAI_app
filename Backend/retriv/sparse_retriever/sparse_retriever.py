...
class SparseRetriever(BaseRetriever):
    ...
    def search(self, query: str, return_docs: bool = True, cutoff: int = 100) -> List:
        ...
        ###
        top_docs_for_terms = {}  # Initialize 10_docs_for_terms dictionary
        for term_id, scores in terms_scores.items():
            term = query_terms[term_id]
            doc_indices = np.argsort(-scores)[:10]
            top_docs_names = []
            for doc_id in doc_indices:
                doc_name = self.id_mapping[doc_id]
                top_docs_names.append(doc_name)
            top_docs_for_terms[term] = top_docs_names
        ###
        unique_doc_ids = self.map_internal_ids_to_original_ids(unique_doc_ids)

        if not return_docs:
            return dict(zip(unique_doc_ids, scores)), top_docs_for_terms

        return self.prepare_results(unique_doc_ids, scores), top_docs_for_terms
